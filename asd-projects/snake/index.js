...
function init() {
  ...
  makeSnakeSquare(10, 10); // create the first snakeSquare for the snake's head and position it on the board

makeSnakeSquare(10, 9); // create the second snakeSquare for the snake's body and position it on the board

makeSnakeSquare(10, 8); // create the third snakeSquare for the snake's body and position it on the board

snake.head = snake.body[0]; // set the snake's head to be the first element in the snake's body
...
console.log("Moving bodyA to bodyB");
setTimeout(() => {
moveBodyAToBodyB(snake.body[1], snake.body[0]);
...
function hasHitWall() {
  ...
  */
 if (snake.head.row < 0 || snake.head.row >= board.rows || snake.head.column < 0 || snake.head.column >= board.columns) {
   return true;
 }
  return false;
}
...
if (snake.body.length === 0) {
  ...
  snake.tail.element.css("background-color", "lightgreen");

  coloIndex = (colorIndex + 1) % snakeColors.length;
}
